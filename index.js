...
app.get('/api/prompts/:id/analytics', async (req, res) => {
  try {
    ...
    const reviews = await reviewCollection.find({ promptId: id }).toArray();
    const bookmarks = await bookmarkCollection.countDocuments({ promptId: id });
    res.send({ prompt, reviews, bookmarks });
  } catch (error) {
    res.status(500).send({ message: "Server error", error: error.message });
  }
});
...
app.get('/api/reviews', async (req, res) => {
  try {
    const result = await reviewCollection.find().sort({ createdAt: -1 }).limit(6).toArray();
    res.send(result);
  } catch (error) {
    res.status(500).send({ message: "Server error", error: error.message });
  }
